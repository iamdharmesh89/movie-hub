...
$url_trending_movie = 'https://api.themoviedb.org/3/tv/popular?api_key=' . $API_KEY . '&language=en-US&page=3';
...
<div class="container-fluide">
    <h3 class="Title-popular">Best Tv Shows</h3>
    <div class="raw">
<?php
error_reporting(0);
for ($i = 0; $i < count($data_trending_json); $i++) {

    $movie_image = $data_trending_json['results'][$i]['backdrop_path'];
    $movie_info = $data_trending_json['results'][$i]['overview'];
    $movie_result = $data_trending_json['results'][$i]['vote_average'];
    $movie_title = $data_trending_json['results'][$i]['original_name'];
  ...
  <div class="card-body">
    ...
    <a href="./section/tv_main_page.php?id='.$movie_id.'" class="btn btn-outline-warning">Watch Now</a>
