...
$url_pupular_movie = 'https://api.themoviedb.org/3/movie/popular?api_key=' . $API_KEY . '&page=3&limit=20';

$data = file_get_contents($url_pupular_movie);
...
<div class="container-fluide">
    <h2 class="title-brand">Atlas Movie <span class="other-text">Hub</span></h2>
    <div class="raw">
    <h3 class="Title-popular">Popular Movies</h3>
    <form action="./section/filter.php" method="GET">
        <input class="search" type="search" name="MOVIE_TITLE" placeholder="search movie">
     </form>   
    </div>
    <div class="raw">
...
for ($i = 0; $i < count($data); $i++) {
    ...
    $movie_image = $data['results'][$i]['poster_path'];
    $movie_info = $data['results'][$i]['overview'];
    $movie_result = $data['results'][$i]['vote_average'];
    $movie_title = $data['results'][$i]['original_title'];
...
<br>
<?php 
include('./section/trending.php');


include('./section/horror.php');



include('./section/tvshow.php');
